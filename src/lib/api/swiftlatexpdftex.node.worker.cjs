...
var initmem = undefined;

var mainfile = "main.tex";

var texlive_endpoint = "https://texlive2.swiftlatex.com/";
...
function compileLaTeXRoutine() {
    ...
    if (status === 0) {
        ...
        try {
            let pdfurl = WORKROOT + "/" + mainfile.substr(0, mainfile.length - 4) + ".pdf";
...
parentPort.onmessage = handleOnMessage;
...
async function kpse_find_file_impl(nameptr, format, _mustexist) {
    ...
    const remote_url = texlive_endpoint + "pdftex/" + cacheKey;
...
function createWasm() {
    ...
    function instantiateArrayBuffer(receiver) {
      const wasmBuffer = nodefs.readFileSync(path.join(__dirname, './swiftlatexpdftex.wasm'));

      return WebAssembly.instantiate(wasmBuffer, info).then(wasmModule => {
        return wasmModule
...
async function _kpse_find_file_js(nameptr, format, mustexist) {
